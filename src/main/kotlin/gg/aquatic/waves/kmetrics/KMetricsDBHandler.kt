...
class KMetricsDBHandler(private val database: Database) {
    ...
    suspend fun batchUpdate(updates: Map<String, Map<UUID, Pair<BigDecimal, String>>>) = newSuspendedTransaction(db = database, context = VirtualsCtx) {
    ...
    suspend fun cleanup(type: PeriodType, amountToKeep: Int) = newSuspendedTransaction(db = database, context = VirtualsCtx) {
        val now = LocalDateTime.now()
